...
class ZenithFB
{
    ...
    public function __construct($bs_id = '')
    {
        ...
        try {
            ...
            $this->db = new FBDB();
            $this->access_token = empty($this->access_token) ? "" : (empty($this->longLivedAccessToken) ? $this->access_token : $this->longLivedAccessToken);
    ...
    public function getFBAccounts()
    {
        try {
            ...
            $msgs = [
                'channel' => '광고API',
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][광고계정] 수신 오류 : " . $ex->getMessage(),
            ];
    ...
    public function getAsyncInsights($all = "false", $date = null, $edate = null, $account_id = null)
    {
        try {
            ...
            $msgs = [
                ...
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][인사이트] {$date}~{$edate} 수신 완료 (실행 시간: {$executionTime}초)",
            ];
            $this->slack->sendMessage($msgs);
            if ($all == "true") {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][인사이트] {$date}~{$edate} 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function updateAllByAccounts() {
        ...
        $msgs = [
            'channel' => $this->slackChannel,
            'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북]비지니스별 캠페인/광고그룹/광고 정보 수신 완료",
        ];
        $this->slack->sendMessage($msgs);
    }
    ...
    public function updateAllByAccount($bs_id=null) {
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북] {$bs_id} 캠페인/광고그룹/광고 정보 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function updateAdCreatives($data = null, $adcreatives = null)
    {
        try {
            ...
            foreach ($adcreatives as $data) {
                ...
                $asset_feed_spec = $data[AdCreativeFields::ASSET_FEED_SPEC] ?? [];
                if(isset($asset_feed_spec['call_to_action_types']) && isset($asset_feed_spec['call_to_action_types'][0]))
                    $data[AdCreativeFields::CALL_TO_ACTION_TYPE] = $asset_feed_spec['call_to_action_types'][0];
                if (isset($data[AdCreativeFields::CALL_TO_ACTION_TYPE]) && in_array($data[AdCreativeFields::CALL_TO_ACTION_TYPE], ["LEARN_MORE", "APPLY_NOW", "SIGN_UP", "SHOP_NOW"])) {
                    ...
                    if(isset($updatedAdCreative['object_type'])){
                        switch ($updatedAdCreative['object_type']) {
                            ...
                            case 'STATUS':
                                ...
                                } else if(isset($asset_feed_spec['link_urls']) && isset($asset_feed_spec['link_urls'][0]) && isset($asset_feed_spec['link_urls'][0]['website_url'])) {
                                    $updatedAdCreative['link'] = $asset_feed_spec['link_urls'][0]['website_url'];
                                }
                                break;
                            case 'VIDEO':
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][소재 정보] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function getAdCreatives($data = null)
    {
        ...
        $fields = [
            ...
            AdCreativeFields::ASSET_FEED_SPEC,
    ...
    public function updateAds($data = null)
    {
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][광고 정보] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function updateAdsets($data = null)
    {
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][광고그룹 정보] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function updateCampaigns($data = null)
    {
        ...
        if (count($_ids)) {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][캠페인 정보] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function getAdLead($from = "-1 day", $to = null)
    {
        try {
            ...
            foreach ($ad_ids->getResultArray() as $row) { //while $row['page_id']
                ...
                if (isset($response['paging'])) {
                    ...
                    while ($url) {
                        $data = $this->getFBRequest_CURL($url);
                        if (isset($data['data'])) {
                            $lead_data = array_merge($lead_data, $data['data']);
                            $result = array_merge($result, $data['data']);
                        }
                        $url = isset($data['paging']['next']) ? $data['paging']['next'] : false;
                    }
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][잠재고객] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function updateAdAccounts()
    {
        ...
        $msgs = [
            'channel' => $this->slackChannel,
            'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][페이스북][광고계정] 수신 완료",
        ];
        $this->slack->sendMessage($msgs);
        return $result;
    }
        ...
        foreach($ads->getResultArray() as $row) {
            ...
            */
            ...
            for($i=0; $i<=23; $i++) { //DB수량이 없어도 지출금액이 갱신되어야하기 때문에 0~23시까지 모두 저장
                ...
                if(preg_match('/cpm/i', $data['media'])) $count = 0;
                if($data['period_ad']) $margin = $spend * ('0.' . $data['period_ad']);
                $data['data'][] = [
                    'hour' => $hour
                    ,'spend' => $spend
